  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  if (XTmrCtr_Initialize(&TimerCounterInst, XPAR_AXI_TIMER_0_DEVICE_ID) != XST_SUCCESS) {
  ...
  {
	  Number_Of_Runs = 1000;
	  printf("Number_Of_Runs set to %d",Number_Of_Runs);
  }
  printf ("\n");
...
#if 1
#ifdef TIMES
  Begin_Time = XTmrCtr_GetValue(&TimerCounterInst, 0);
#endif
#ifdef TIME
...
#if 1
#ifdef TIMES
  End_Time = XTmrCtr_GetValue(&TimerCounterInst, 0);
#endif
#ifdef TIME
...
#if 1
    int tmp_int;
    tmp_int = Microseconds;
    printf ("Microseconds for one run through Dhrystone: %d.%02d \n", tmp_int, (unsigned int)((Microseconds * 100) - (tmp_int * 100)));
    tmp_int = Dhrystones_Per_Second;
    printf ("Dhrystones per Second:                      %d.%02d \n", tmp_int, (unsigned int)((Dhrystones_Per_Second * 100) - (tmp_int * 100)));
    printf ("\n");
#else
    printf("Completed\n");
#endif
